...
def train_and_evaluate(
    ...
    with mlflow.start_run():
        ...
        for class_name in metrics['classification_report']:
            ...
            f1_score = metrics['classification_report'][class_name]['f1-score']
            mlflow.log_metric(f"{class_name}_precision", precision)
            mlflow.log_metric(f"{class_name}_recall", recall)
            mlflow.log_metric(f"{class_name}_f1", f1_score)
        
        # Plot feature importance
        feature_importance_df = plot_feature_importance(model, feature_names, reports_dir)
        mlflow.log_artifact(os.path.join(reports_dir, 'feature_importance.png'))
        
        # Plot confusion matrix
        plot_confusion_matrix(np.array(metrics['confusion_matrix']), class_names, reports_dir)
        ...
        latest_model_path = os.path.join(model_dir, 'crop_model_latest.joblib')
